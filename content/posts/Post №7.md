...
**It's finally time to return to VS.**
...
*In the next video, we will build our ribbon.*

# 7. Create a PushButton
- Let's set up a button we can run our command from (we had this before, but we're doing it the typical way vs the Nic3Point way).
- We will create a tab, a panel, a button and finally set up a basic command that runs when we press it.

*We will learn about icons/tooltips later.*
...
*We will follow on with tooltips/icons in our next lesson.*
